...
import { CardBox, HeaderHome, HomeStyle, MenuBox, Select } from "./HomeStyle";
...
export const Home = (props) => {
  ...
  const onChangeOrdination = (event) => {
    //No evento onChangeOrdination, atualizei o estado ordination e chamei uma função de ordenação dos produtos. Essa função é responsável por ordenar a lista de produtos com base no critério selecionado.
  ...
  const adicionaItensCarrinho = (produto) => {
    ...
    if (novoProduto === undefined) {
      // verifica se o produto dentro do novoProduto Existe
      ...
      const novoCarrinho = props.cart.map((produtoDoCarrinho) => {
        // Esse else serve para que se o produto já existe no carrinho só adiciona quantidade
  ...
  const sortProducts = (ordination) => {
    //Criei a função sortProducts que será responsável por ordenar a lista de produtos com base no critério selecionado. Essa função cria uma cópia da lista de produtos (props.productList) e a ordena com base no critério selecionado. Em seguida, atualiza o estado sortedProducts com a lista ordenada.
  ...
  const listaDeProdutos = props.productList.map((item) => {
    // Atualizei a variável listaDeProdutos para usar a lista ordenada sortedProducts em vez de props.productList.
    QuantidadeProdutos++;
